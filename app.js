...
// mongodb connection
mongoose.connect(`${config[env].DB_HOST}:${config[env].DB_PORT}/${config[env].DB_NAME}`, {
  useNewUrlParser: true,
});
mongoose.connection.on('error', err => log.error(`MongoDB connection Error: ${err}`));
mongoose.connection.on('connected', () => {
  log.info(`MongoDB Connected on: ${mongoose.connection.host}:${mongoose.connection.port}/${mongoose.connection.name}`);
});
